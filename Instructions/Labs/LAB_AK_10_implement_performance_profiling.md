...
- ContosoOnlineStore：这是一个可处理客户订单的电子商务应用程序。 该应用程序包含带搜索功能的产品目录管理、带库存预留的库存跟踪、带验证与收据的订单处理、电子邮件通知服务以及安全验证功能。 应用程序使用新式 .NET 体系结构模式，包括依赖项注入、结构化日志记录和配置管理，但包含反映真实场景情况的性能瓶颈。
